...
<h5 align="right">
    San Vicente, <?php echo date('d'); ?> de <?php $meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
...
<p align="justify">
    <h><br>Reciba cordiales saludos y deseos de &eacute;xitos en sus labores diarias.</h>
</p>
<p align="justify">


    <h>
        <!-- En atención a lo establecido en el art.205 del Reglamento General de la Gestión Académico - Administrativo de la
                    ...
                    @if ($ase->iddocente == $doc->iddocente)
                        @foreach ($personas as $per)
                            @if ($per->idpersona == $doc->idpersona)
                                ...
                                <tr>
                                    <td></td>
                                    <td>{{ $doc->titulo }}{{ ' ' }}{{ $per->nombres }}{{ ' ' }}{{ $per->apellidos }}
                                    </td>
                                    <td></td>
...
<p align="justify">
    <h>Se anexa el escrito presentado por el estudiante en el que expresa su desici&oacute;n de renunciar al proceso en
        el que actualmente esta inscrito.</h>
</p>
