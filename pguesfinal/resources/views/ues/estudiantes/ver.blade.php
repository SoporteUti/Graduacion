...
role="dialog" tabindex="-1" id="modal-ver-{{$per->idpersona}}">
  {{Form::Open(array('action'=>array('EstudianteController@show',$est->curriculum),'method'=>'PATCH'))}}
 ...
 @endforeach
  ...
  <div class="col-lg-6 col-md-6 col-xs-12 col-sm-6" >
             <div class="form-group"> 
                ...
                <div class="input-group">
                    <span class="input-group-addon"><i class="fa fa-text-width"></i></span>
                      <select disabled="" name="ciclo" id="ciclo" class="form-inline form-control">
...
<select disabled="" name="anio" id="anio" class="form-inline form-control">
        @foreach($estudiante as $est)
                    @if($est->idpersona==$per->idpersona)
                   <?php
//$rest = substr("abcdef", -1);    // devuelve "f"
...
<a href="{{asset('storage/cartas_egreso/'.$est->carta)}}" target="_blank" >
