...
<body>
  ...
  <td><center><h4>Universidad de El Salvador<br>Facultad Multidisciplinaria Paracentral </h4><h5>Listado de Docentes Inactivos  </h5></center></td>
          ...
          <?php foreach($persona as $per) { ?>
            <?php foreach($docentes as $doc) { ?>
              <?php if($per->idpersona==$doc->idpersona) {?>
                <?php if($per->condicion==False) {?>
          <tr><!--fila simple-->
            <td><?php echo $cont++ ?></td>
            <td><?= $doc->titulo ?>  <?=$per->nombres ?></td><!--celda sencilla-->
