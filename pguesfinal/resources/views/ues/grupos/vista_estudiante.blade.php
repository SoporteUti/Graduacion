...
<th>Promedio</th>
       ...
       <tbody>  @foreach($mostraintegrante as $mint)
                ...
                @if($mint->idestudiante==$est->idestudiante)
                @foreach($personas as $per)
                @if($per->idpersona == $est->idpersona && Auth::user()->idpersona==$per->idpersona)
         <tr>
...
@if($not->idestudiante==$est->idestudiante && $not->idetapa==$j )
...
@foreach($notas as $not)
@if($not->idestudiante==$est->idestudiante &&  $not->idetapa==$et->idetapa )
