...
<li><a href="" data-target="#solicitud-4" data-toggle="modal">Ratificación Tribunal Calificador</a></li>
        ...
        <ul class="dropdown-menu">
          ...
          <li><a href="#" onclick="ventanaP('{{ $grupos->idgrupo}}')" >Imprimir Expediente</a></li>
  ...
  <div class="col-lg-3 col-md-3 col-xs-3 col-sm-3" >
    ...
    <li class="nav-item" id="$et->idetapa" >
                    ...
                    <a o class="nav-link" data-toggle="tab" href="#page-{{$et->idetapa}}"><strong>{{$et->idnombreetapa}}</strong></a>
                    <?php $var++; ?>
                </li>
    @endif
    @endforeach
 ...
 <div class="tab-content">
  ...
  @for ($i = 1; $i <=$var; $i++)
                 ...
                 <tbody >
                  ...
                  <td ><a data-backdrop="static" data-keyboard="false" href=""  data-target="#imprimirc-{{$sol->idsolicitud}}-{{$gs->idgrupsol}}" data-toggle="modal">
                    <button  class="btn btn-warning"><i class="fa fa-print"></i></button></a>
@if($gs->estado==3 && $gs->condicion==true )
<a href=""  data-target="#cancelar-{{$gs->idgrupsol}}" data-toggle="modal"> <button  class="btn btn-danger"><i class="fa fa-ban"></i></button></a>
@include('ues.grupos.cancelarsolicitud')
 ...
 @if($gs->condicion==false )
<a href=""  data-target="#motivo-{{$gs->idgrupsol}}" data-toggle="modal"> <button  class="btn btn-danger"><i class="fa fa-question-circle 
"></i></button></a>

                  
                
                  @endif   
<a href=""   data-target="#verpdfs-{{$gs->idgrupsol}}" data-toggle="modal">
...
<div class="modal-dialog">
 ...
 <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12" >
   ...
   <th>Etapa {{$et->idetapa}} ({{$pr->porcentaje}}%)</th>
    <?php $cet++; ?>
    @endif
    @endforeach
    @endif
...
<th>Promedio</th>
       ...
       <tbody>  @foreach($mostraintegrante as $mint)
         ...
         <tr>
               ...
               <td>{{$est->carnet." ".$per->nombres." ".$per->apellidos}}</td>
@for ($j = 1; $j <=$cet; $j++)
<td>
...
{{Form::Open(array('action'=>array('solicitudController@impugnacionResultados'),'route'=>['ues.solicitudes.impugnacionResultados'], 'method'=>'post', 'files' =>'true'))}}
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header" style="background:#00a65a; color:white">
    ...
    <div class="modal-body">
      <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12" >
          ...
          <div class="form-group"> 
                ...
                @foreach($personas as $per)
                @if($per->idpersona == $est->idpersona)
                 <option  value="{{$est->idestudiante}}">{{$est->carnet." ".$per->nombres." ".$per->apellidos}}</option>
                @endif
                @endforeach
                @endif
                @endforeach
...
{{Form::Open(array('action'=>array('GrupoController@gnotas'),'route'=>['ues.grupos.gnotas'], 'method'=>'post', 'files' =>'true'))}}
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header" style="background:#00a65a; color:white">
...
<!-- guardar etapa activa 1 al crear el grupo no al generar la solicitud de ap. tema -->
    ...
    @if($et->idetapa==$ea->idnuevaetapa)
                    <input readonly="" type="text" value="{{$et->idnombreetapa}}" name="etapa" id="etapa" class="form-control"  >   
                    <input  type="hidden" value="{{$et->idetapa}}" name="idetapa" id="idetapa" class="form-control"  > 
                    @endif
                    @endif
                    @endforeach
                </div>          
          ...
          <div class="form-group"> 
                ...
                <div class="col-lg-3 col-md-3 col-xs-3 col-sm-3">
                <input   type="number"  min="0"  max="10" step = ".01" id="{{$est->idestudiante}}" name="{{$est->idestudiante}}" class="form-control"    value="" placeholder="Nota">
                </div> 
                <br>
                @endif
                @endforeach
...
{{Form::Open(array('action'=>array('GrupoController@pdfNotas'),'route'=>['ues.grupos.impNotas'], 'method'=>'post', 'files' =>'true'))}}
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header" style="background:#00a65a; color:white">
 ...
 <div hidden="" class="col-lg-12 col-md-12 col-xs-12 col-sm-12" >
            ...
            <div hidden="" class="col-lg-12 col-md-12 col-xs-12 col-sm-12" >
                ...
                <label>numero de etapas</label>
                    <div class="input-group">                   
                        <span class="input-group-addon"><i class="fa fa-info"></i></span>
                        <input readonly="" type="text" value="{{$var}}" name="idne" id="idne" class="form-control"  >
                    </div>          
                </div>
                </div>
...
<option value="{{$et->idnuevaetapa}}">{{$et->idnombreetapa}}
...
{{Form::Open(array('action'=>array('GrupoController@editarnotas'),'route'=>['ues.grupos.editarnotas'], 'method'=>'post', 'files' =>'true'))}}
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header" style="background:#00a65a; color:white">
...
@foreach($tiproceso as $tip)
  ...
  @if($idetapa>0 && $et->idetapa<=$idetapa )
                  <option value="{{$et->idetapa}}">{{$et->idnombreetapa}}</option>
                          @endif
          @endif
    @endforeach
...
{{Form::Open(array('action'=>array('solicitudController@renuncia'),'route'=>['ues.solicitudes.renuncia'], 'method'=>'post', 'files' =>'true'))}}
<div class="modal-dialog">
  <div class="modal-content">
    <div class="modal-header" style="background:#00a65a; color:white">
...
@foreach($mostraintegrante as $mint)
                ...
                @foreach($personas as $per)
                @if($per->idpersona == $est->idpersona)
                  <option value="{{$est->idestudiante}}">{{$per->nombres}} {{$per->apellidos}}</option>
                             @endif
                @endforeach
                @endif
                @endforeach
...
{
ventana1=window.open("{{ url('ues/grupos/notasG') }}"+"/"+ id,'scrollbars=yes,width=800,height=1000,titlebar=no'); 
}
</script>
...
{
ventana1=window.open("{{ url('ues/grupos/cronogramapdf') }}"+"/"+ id,'scrollbars=yes,width=800,height=1000,titlebar=no'); 
}
</script>
