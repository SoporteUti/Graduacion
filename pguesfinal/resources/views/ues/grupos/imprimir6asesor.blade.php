...
{{Form::Open(array('action'=>array('solicitudpicController@spicontaAsesor'),'route'=>['ues.solicitudesconta.spicontaAsesor'], 'method'=>'post', 'files' =>'true'))}}
            ...
            <label>Motivo</label>
                <div class="input-group">                   
                    <span class="input-group-addon"><i class="fa fa-info"></i></span>
                     ...
                     @foreach($cambiotema as $ct)         
                        @if( $gs->idgrupsol == $ct->idgrupsol) 
                    <input readonly="" name="motivo"  value="{{$ct->motivo}}" id="motivo" class="form-control" >
                     @endif
                    @endforeach
